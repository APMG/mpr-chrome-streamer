...
define([], function () {
    ...
    isStream = function () {
        if (_currentPlayItem._content.mode === 'live_audio')
            return true;
        return false;
    };
    // test to see if the audio is static (fix length) audio
    isStatic = function () {
        if (_currentPlayItem._content.mode === 'audio')
            return true;
        return false;
    };
    ...
    wtMediaMETA = function (meta) {
        ...
        if (typeof meta.is_underwriting !== 'undefined') {
            if (meta.is_underwriting)
                _currentPlayItem._content.underwriting = true;
            else
                currentPlayItem._content.underwriting = false;
        }
        ;
    };
    // END wtMediaMETA
    ...
    wtPosUpdate = function (meta) {
        ...
        if (beacon.lastBeacon === 0)
            beacon.lastBeacon = playTime.currentVal;
        if (playTime.currentVal > beacon1end * 1000)
            beaconRate = beaconRate2;
    ...
    wtSendData = function (vars) {
        ...
        for (var c = 0; c < vars.length; c++) {
            ...
            if (typeof (vars[c].value) === 'number')
                myArgs.push(vars[c].value.toString());
            else if (typeof (vars[c].value) === 'string')
                myArgs.push(vars[c].value.replace(/<\/?[a-z][a-z0-9]*[^<>]*>/ig, ""));
            else
                myArgs.push('unknown');
        }
        dcsMultiTrack.apply(this, myArgs);
    };
    wtClipInfo = function (event) {
        ...
        var tags = {tag: null, value: null};
        tags.tag = "WT.clip_n";
        if (myData._content.program)
            tags.value = myData._content.program;
        else
            tags.value = document.title;
        if (_currentPlayItem._content.underwriting)
            tags.value = 'underwriting:' + tags.value;
        myTags.push(tags);
        ...
        if (event === 'QUARTILE') {
            ...
            myTags.push(tags);
            var tags = {tag: null, value: null};
            if (_currentPlayItem._quartile === 1)
                tags.value = 1;
            if (_currentPlayItem._quartile === 2)
                tags.value = 3;
            if (_currentPlayItem._quartile === 3)
                tags.value = 5;
            if (_currentPlayItem._quartile === 4)
                tags.value = 9;
            tags.tag = "WT.clip_nq";
            myTags.push(tags);
        }
